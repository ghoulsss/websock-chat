...
console.log("CSS загружен:", document.styleSheets.length > 0);
...
document
	...
	.addEventListener("click", async (event) => {
		...
		const password_check = document.querySelectorAll(
			'#registerForm input[type="password"]',
		)[1].value;
		...
		await handleFormSubmit("register", "register/", [
			email,
			name,
			password,
			password_check,
